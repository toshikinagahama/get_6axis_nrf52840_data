...
import 'package:get_6axis_nrf52840_data/data/model/measure_raw_data.dart';
...
class BleNotifier extends ChangeNotifier {
  ...
  static const dataServiceUUID = "4fafc202-1fb5-459e-8fcc-c5c9c331914b";
  static const dataRxCharaUUID = "beb5483f-36e1-4688-b7f5-ea07361b26a8";
  static const dataTxCharaUUID = "beb5483e-36e1-4688-b7f5-ea07361b26a8";

  bool isBatteryServiceFound = false;
  bool isDataServiceFound = false;
  BluetoothDevice? device;
  BluetoothCharacteristic? dataRxChara;
  BluetoothCharacteristic? dataTxChara;
  StreamSubscription? dataRxCharaStream;
  StreamSubscription? dataTxCharaStream;
  ...
  void disconnect() {
    ...
    isConnected = false;
    if (dataRxCharaStream != null) dataRxCharaStream!.cancel();
    dataRxChara = null;
    if (dataTxCharaStream != null) dataTxCharaStream!.cancel();
    dataTxChara = null;
        ...
        case dataServiceUUID: //データサービスのとき
          ...
          for (var chara in service.characteristics) {
            ...
            switch (chara.uuid.toString()) {
              case dataRxCharaUUID:
                dataRxChara = chara;
                await Future.delayed(const Duration(milliseconds: 200));
                try {
                  await dataRxChara!.setNotifyValue(true);
                  dataRxCharaStream =
                      dataRxChara!.lastValueStream.listen((value) {
                    //logger.i(value);
  ...
  void startMeas() async {
    ...
    //コマンド送信
    if (dataTxChara == null) return;
    var command = [0x01, 0x00];
    dataTxChara!.write(command, withoutResponse: false);
  }
